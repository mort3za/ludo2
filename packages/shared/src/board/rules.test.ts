...
describe("isSafeSquare", () => {
  it("S=4 start squares are safe", () => {
    ...
    expect(isSafeSquare("T/12", 4)).toBe(true); // seat 2
    expect(isSafeSquare("T/23", 4)).toBe(true); // seat 3
    expect(isSafeSquare("T/34", 4)).toBe(true); // seat 4
  });

  it("non-start track squares are not safe", () => {
    expect(isSafeSquare("T/2", 4)).toBe(false);
    expect(isSafeSquare("T/11", 4)).toBe(false);
    expect(isSafeSquare("T/44", 4)).toBe(false);
  });
  ...
  it("S=6 has exactly 6 safe squares", () => {
    ...
    expect(isSafeSquare("T/12", 6)).toBe(true); // seat 2
    expect(isSafeSquare("T/23", 6)).toBe(true); // seat 3
    expect(isSafeSquare("T/34", 6)).toBe(true); // seat 4
    expect(isSafeSquare("T/45", 6)).toBe(true); // seat 5
    expect(isSafeSquare("T/56", 6)).toBe(true); // seat 6
    // not safe
    expect(isSafeSquare("T/66", 6)).toBe(false);
  });

  it("S=8 has exactly 8 safe squares", () => {
    expect(isSafeSquare("T/1", 8)).toBe(true);
    expect(isSafeSquare("T/12", 8)).toBe(true);
    expect(isSafeSquare("T/23", 8)).toBe(true);
    expect(isSafeSquare("T/34", 8)).toBe(true);
    expect(isSafeSquare("T/45", 8)).toBe(true);
    expect(isSafeSquare("T/56", 8)).toBe(true);
    expect(isSafeSquare("T/67", 8)).toBe(true);
    expect(isSafeSquare("T/78", 8)).toBe(true);
  });
});
...
describe("isOvershoot", () => {
  ...
  it("mixed track-to-home within bounds is not overshoot", () => {
    expect(isOvershoot(["T/11", "H/2/1"])).toBe(false);
  });

  it("mixed track-to-home overshooting is overshoot", () => {
    expect(isOvershoot(["T/44", "H/1/1", "H/1/2", "H/1/3", "H/1/4", "H/1/5"])).toBe(true);
  });
